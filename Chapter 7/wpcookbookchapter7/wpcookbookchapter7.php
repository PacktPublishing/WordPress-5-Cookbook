...
function wpccp_chapter7_save_book_data($post_id, $post, $update){
  ...
  $wpccp_book_price = isset( $_POST['wpccp_book_price'] ) ? (int) ($_POST['wpccp_book_price']) : '';
...
function wpccp_chapter7_access_database(){
  ...
  switch ($action) {
    // Step 2 code should be placed in next line
    case 'insert_post':
     $post_data = array(
...
function wpccp_chapter7_rest_api_post_list(){
  $request = wp_remote_get( 'http://www.yoursite.com/wp-json/wp/v2/posts?_fields=author,id,excerpt,title,link' );
